...
export function SettingsPage({ settings, onSave }: { settings: Settings; onSave: (settings: Settings) => Promise<void> }) {
  ...
  return <main className="page scroll"><div className="page-header"><div><span className="eyebrow">Configuration</span><h1>Settings</h1><p>Stored locally as TOML in the application config directory.</p></div></div>
    <section className="settings-card">
      <label>Provider format<select value={form.providerFormat} onChange={e => update("providerFormat", e.target.value as Settings["providerFormat"])}><option value="openai">OpenAI Chat Completions</option><option value="openai_responses">OpenAI Responses API</option><option value="anthropic">Anthropic Messages</option></select></label>
      <label>API key<input type="password" value={form.apiKey} onChange={e => update("apiKey", e.target.value)} placeholder="sk-…" /></label>
      <label>Base URL<input value={form.baseUrl} onChange={e => update("baseUrl", e.target.value)} placeholder={form.providerFormat === "anthropic" ? "https://api.anthropic.com" : "https://api.openai.com/v1"} /></label>
      <label>Model<input value={form.model} onChange={e => update("model", e.target.value)} placeholder={form.providerFormat === "anthropic" ? "claude-sonnet-4-5" : "gpt-5"} /></label>
