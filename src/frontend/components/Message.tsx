...
export function Bubble({ message, onApprove }: BubbleProps) {
  ...
  return (
    <div className="flex flex-col items-start gap-2">
      ...
      {message.error && <div className="max-w-[85%] rounded-xl2 border border-red-300 bg-red-50 px-4 py-2.5 text-sm text-red-700 dark:border-red-800 dark:bg-red-900/20 dark:text-red-300">Error: {message.error}</div>}
    </div>
  );
}
